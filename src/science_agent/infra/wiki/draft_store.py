...
class JsonWikiDraftStore:
    ...
    async def list_drafts(self) -> list[WikiChangeSet]:
        return await asyncio.to_thread(self._list_drafts)
    ...
    def _save(self, changeset: WikiChangeSet) -> None:
        with self._lock:
            ...
            if path.is_file():
                ...
                if self._comparable(existing) != self._comparable(payload):
    ...
    def _list_drafts(self) -> list[WikiChangeSet]:
